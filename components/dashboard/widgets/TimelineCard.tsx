       ...
       <div>
          ...
          <div className="bg-white dark:bg-gray-900 p-3 rounded-xl border border-gray-100 dark:border-gray-800 shadow-sm min-w-[200px] hover:shadow-md transition-shadow">
             <div className="flex justify-between items-start mb-2">
               ...
               {status === 'live' && <span className="text-[10px] font-bold bg-green-100 text-green-700 px-1.5 py-0.5 rounded animate-pulse">{t('dashboard.liveNow')}</span>}
             </div>
             ...
             <div className="flex justify-between items-center text-xs border-t border-gray-50 dark:border-gray-800 pt-2">
                <span className="font-bold text-gray-700 dark:text-gray-300">{attendees} {t('dashboard.checkins')}</span>
                {unpaid > 0 ? <span className="text-red-500 font-bold">{unpaid} {t('dashboard.failedPayments')}</span> : <span className="text-green-600 font-bold">OK</span>}
             </div>
          </div>
       </div>
    </div>
